	...
	public static void main(String[] args) throws IOException {
		...
		for (File inputSubDir : inputSubDirsToProcess) {
			...
			while (inputData.size() > 0) {
				...
				System.out.println("Done processing data for valve: " + valveName + ". Writing data to: " + filePath + fileName);
				...
				if (!new File(filePath + fileName).exists()) {
					...
					Files.write(Paths.get(filePath + fileName), "Default Datalog Configuration,2014-12-03 12:36:04\n".getBytes(),
				...
				Files.write(Paths.get(filePath + fileName), writeToFile.toString().getBytes(), StandardOpenOption.APPEND);
			}

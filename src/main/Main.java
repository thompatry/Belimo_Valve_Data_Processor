		...
		for (File inputSubDir : inputSubDirsToProcess) {
			...
			for (File file : inputSubDir.listFiles()) {
				...
				for (String line : data) {
					if ((line.split(",")).length == 5) // Validate the register has 5 columns. Else, don't add it to the inputData, it is corrupt
						inputData.add(line);
				}
			}
			...
			while (inputData.size() > 0) {
				Date date = new Date();
				SimpleDateFormat fileNameFormatter = new SimpleDateFormat("yyyy_MM_MMM");
				...
				System.out.println(
						"Done processing data for valve: " + valveName + ". Writing data to: " + filePath + fileName);
				...
				if (!new File(filePath + fileName).exists()) {
					...
					Files.write(Paths.get(filePath + fileName),
							"Default Datalog Configuration,2014-12-03 12:36:04\n".getBytes(),
				...
				Files.write(Paths.get(filePath + fileName), writeToFile.toString().getBytes(),
						StandardOpenOption.APPEND);
			}
